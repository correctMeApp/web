...
export default function Pricing({ product, prices }: Props) {
  ...
  const handleStripeCheckout = async (price: Price) => {
    ...
    if (!isLoggedIn) {
      ...
      const redirectUrl = `pricing?priceId=${price.id}`;
      return router.replace(`/auth/signin?redirect=${redirectUrl}`);
    }
  ...
  useEffect(() => {
    if (checkoutPriceId) {
      const price = prices.find(price => price.id === checkoutPriceId);
  
      if (price) {
        handleStripeCheckout(price);
      }
    }
  }, [checkoutPriceId, prices, handleStripeCheckout]);
            ...
            <div className="relative self-center mt-6 bg-slate-950 rounded-lg p-0.5 flex sm:mt-8 border border-zinc-800">
              {intervals.includes(PricingPlanInterval.Month) && (
                <button
                  ...
                  } rounded-md m-1 py-2 text-sm font-medium whitespace-nowrap focus:outline-none focus:ring-2 sm:w-auto sm:px-8`}
                >
                  Monthly
                </button>
              )}
            ...
            {prices.map((price) => {
              ...
              return (
                <div
                  key={product.id}
                  className={cn(
                    'flex flex-col rounded-lg shadow-sm divide-y divide-zinc-600 bg-slate-800',
